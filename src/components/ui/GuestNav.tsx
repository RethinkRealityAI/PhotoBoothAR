...
function keyForPath(path: string): NavKey | null {
  if (path.startsWith('/wall')) return 'wall';
  if (path.startsWith('/upload')) return 'upload';
  if (path.startsWith('/me') || path.startsWith('/gallery')) return 'photos';
  if (path.startsWith('/booth') || path.startsWith('/experience') || path === '/') return 'booth';
  return null;
}
...
export default function GuestNav({ current, extras, bottomOnMobile = true, className = '' }: Props) {
  ...
  const active = current ?? keyForPath(pathname);
