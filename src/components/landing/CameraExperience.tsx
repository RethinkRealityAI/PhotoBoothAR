 ...
 *
 * CameraExperience — the landing demo's REAL booth camera, as a reusable
 * screen: live camera → WebGL effect → face-tracked 3D prop → frame overlay,
 * with capture, review (beam / save / share / retake) and a camera-app
 * control deck (category tabs Frame · Effect · 3D over an orb carousel over
 * the shutter). Extracted from ShowcasePhone so two surfaces can run it:
 *
 *   • ShowcasePhone — inside the landing scene's phone mockup;
 *   • BeamDemoPhone — full-screen on a visitor's REAL phone (/beam/:id),
 *     where "beam" broadcasts the shot to the desktop's live wall.
 *
 * The host owns WHEN the camera runs (`enabled`) and what "beam" means
 * (`beamLabel` / `onBeam`); this component owns everything on the screen.
 * Fills its parent: render inside a `relative` box with a set size.
 *
 * The feed renders in a 9:16 viewfinder letterboxed inside the host box
 * (StageCanvas composites 9:16 — matching it keeps the AR frame's edges
 * visible and Overlay3D's prop canvas aligned with the feed).
 */
import {
...
/* ── The seven-hue brand spectrum, shared by the whole showcase. ──────── */
...
export interface CameraExperienceProps {
  /** Camera stream runs only while true — hosts release the device camera
   *  by flipping this off (state changes, page navigation, beaming…). */
  enabled: boolean;
  /** Unique DOM id for the hidden <video> (Overlay3D binds to it). */
  videoId: string;
  /** Label of the review screen's primary action. */
  beamLabel: string;
  onBeam: (shot: string) => void;
  /** X button in the top bar; omit to hide it. */
  onClose?: () => void;
  /** Ref to the 9:16 viewfinder — the beam ceremony measures its origin. */
  screenRef?: React.Ref<HTMLDivElement>;
}

export default function CameraExperience({
  enabled, videoId, beamLabel, onBeam, onClose, screenRef,
  ...
  const { videoRef, ready, error, retry, facingMode, flipCamera, canFlip } = useCameraStream(enabled, false);
  ...
  useEffect(() => {
    if (!enabled && shot !== null) setShot(null);
  }, [enabled, shot]);
  ...
  const beam = useCallback(() => {
    if (shot === null) return;
    onBeam(shot);
    setShot(null); // the host takes over the ceremony
  }, [shot, onBeam]);
  ...
  return (
    ...
    >
      ...
      {error === null && (
        <>
          {/* 9:16 viewfinder, letterboxed camera-app style in the host box:
              StageCanvas composites a 9:16 frame, so matching its aspect
              keeps the AR frame's side edges visible (object-cover in a
              taller screen would crop them) and keeps Overlay3D's prop
              canvas aligned with the feed. */}
          <div ref={screenRef} className="absolute inset-x-0 top-1/2 aspect-[9/16] -translate-y-1/2 overflow-hidden">
            ...
            {propId !== null && anchor && (
              <Overlay3D proceduralId={propId} anchor={anchor} videoId={videoId} mirror={mirror} />
            )}
            <StageCanvas
              ref={stageRef}
              videoRef={videoRef}
