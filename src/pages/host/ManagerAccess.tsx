    ...
    <div className="absolute inset-0 overflow-y-auto hide-scrollbar">
      ...
      <div className="relative z-10 p-6 md:p-10 flex flex-col gap-8 max-w-2xl mx-auto">
        <header className="animate-rise-in">
          <p className="font-label uppercase tracking-luxe text-[9px] text-brand-muted/40 mb-1">Studio</p>
        ...
        {/* One-time reveal */}
        {reveal && (
          <section className="liquid-glass rounded-2xl border border-accent/40 p-6 animate-rise-in">
            <div className="flex items-start justify-between gap-3 mb-3">
              <h2 className="font-label uppercase tracking-luxe text-[10px] text-accent-2 flex items-center gap-2">
                <KeyRound className="w-3.5 h-3.5" /> {reveal.label} — share link
              </h2>
              <button onClick={() => setReveal(null)} className="text-brand-muted/40 hover:text-brand-fg transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex items-center gap-2">
              <p className="flex-1 font-mono text-[11px] text-brand-muted/80 break-all bg-brand-bg/50 rounded-lg px-3 py-2.5">
                {reveal.url}
              </p>
              <button
        ...
        {/* Create form */}
        <section className="liquid-glass rounded-2xl border border-accent/20 p-6 animate-rise-in">
          <h2 className="font-label uppercase tracking-luxe text-[10px] text-accent-2 mb-4">New access link</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="flex flex-col gap-1.5">
              <span className="font-label uppercase tracking-widest text-[9px] text-brand-muted/40">Label</span>
              <input
                value={label}
                onChange={(e) => setLabel(e.target.value)}
                placeholder="Door staff"
                className="bg-brand-surface/80 border border-accent-3/30 rounded-lg px-3 py-2.5 font-sans text-sm text-brand-fg/90 outline-none focus:border-accent/55 transition-colors"
              />
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="font-label uppercase tracking-widest text-[9px] text-brand-muted/40">Expires (optional)</span>
              <input
                type="datetime-local"
                value={expiry}
          ...
          <button
            onClick={create}
            disabled={creating}
            className="mt-4 w-full py-3 bg-foil text-white font-label uppercase tracking-luxe text-[11px] font-bold rounded-xl glow-accent hover:scale-[1.01] transition-transform disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        {/* Existing tokens */}
        <section className="liquid-glass rounded-2xl border border-accent/15 p-6 animate-rise-in">
          <h2 className="font-label uppercase tracking-luxe text-[10px] text-accent-2 mb-4">
            ...
            <div className="divide-y divide-white/5">
              {tokens.map((t) => (
                <div key={t.id} className="flex items-center gap-3 py-3">
                  <KeyRound className="w-4 h-4 text-accent/60 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-sans text-sm text-brand-fg truncate">{t.label ?? 'Manager link'}</p>
                    <p className="font-sans text-[10px] text-brand-muted/35">
                  ...
                  {confirmRevoke === t.id ? (
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => setConfirmRevoke(null)} className="p-1.5 rounded-lg liquid-glass text-brand-muted/40 hover:text-brand-fg transition-colors">
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => revoke(t.id)} className="p-1.5 liquid-glass rounded-lg text-brand-muted/30 hover:text-red-400 transition-colors" title="Revoke">
