...
function dist2d(a: HandPoint, b: HandPoint): number {
...
function extensionRatio(l: HandPoint[], mcp: number, tip: number): number {
  const base = dist2d(l[mcp], l[0]);
  if (base < 1e-6) return 0;
  return dist2d(l[tip], l[0]) / base;
}

function extended(l: HandPoint[], f: (typeof FINGERS)[number]): number {
  return smoothstep(1.15, 1.75, extensionRatio(l, f.mcp, f.tip));
}

function curled(l: HandPoint[], f: (typeof FINGERS)[number]): number {
  return 1 - smoothstep(0.95, 1.45, extensionRatio(l, f.mcp, f.tip));
}

/** Per-hand raw scores. Exported for tests; callers use handGestureScores. */
export function scoreOneHand(hand: HandSample, face: FaceKeypoints | null): Record<string, number> {
  const l = hand.landmarks;
  const out = zeroScores();

  const handSpan = dist2d(l[0], l[9]); // wrist → middle MCP
  const knuckleSpan = dist2d(l[5], l[17]); // index MCP → pinky MCP
  if (handSpan < 1e-6 || knuckleSpan < 1e-6) return out;

  const ext = FINGERS.map((f) => extended(l, f));
  const curl = FINGERS.map((f) => curled(l, f));
  ...
  const thumbTucked = 1 - smoothstep(0.9, 1.4, dist2d(l[4], l[17]) / knuckleSpan);
  ...
  const spread = smoothstep(0.55, 0.95, dist2d(l[8], l[20]) / knuckleSpan);
  out.palmOpen = Math.min(...ext) * spread;

  // pinch: thumb tip meets index tip, gated on the index actually being out
  // (a fist also puts the thumb near the index root).
  out.pinch = (1 - smoothstep(0.18, 0.42, dist2d(l[4], l[8]) / handSpan)) * ext[0];

  // peace: index+middle out, ring+pinky curled, with a visible V gap.
  const vGap = smoothstep(0.25, 0.6, dist2d(l[8], l[12]) / handSpan);
  ...
  if (face) {
    const faceH = Math.hypot(face.forehead.x - face.chin.x, face.forehead.y - face.chin.y);
    if (faceH > 1e-6) {
      ...
      if (sizeOk >= 0.2 && sizeOk <= 1.1) {
        ...
        for (const t of FINGERTIPS) {
          for (const ear of [face.leftEar, face.rightEar]) {
            const d = Math.hypot(l[t].x - ear.x, l[t].y - ear.y);
            if (d < best) best = d;
          }
        }
...
export function handGestureScores(
  ...
  for (const hand of hands) {
    ...
    const s = scoreOneHand(hand, face);
...
export function handAnchor(
  ...
  for (const hand of hands) {
    ...
    const span = dist2d(hand.landmarks[0], hand.landmarks[9]);
    if (
      best === null ||
