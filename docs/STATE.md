...
## Now
DONE + verified (2026-07-10, round 2 — "next-level" batch): (1) CROSS-DEVICE DEMO — QR panel in InteractiveShowcase (viewport-gated channel: no socket until seen; hidden <lg) → /beam/:channelId (BeamDemoPhone: consent copy — the ONE off-device photo path, spelled out) → CameraExperience (extracted from ShowcasePhone, reused by both) → downscaleShot → demoBeamTransport (Supabase Realtime broadcast for r… ids; BroadcastChannel for L… ids via ?beamlocal=1) → desktop plays remote BeamStrike + "From your phone" polaroid WITHOUT touching local appState. src/lib/demoBeam.ts pure (+12 tests). (2) ParticleBeam.tsx — 13.7k-point WebGL dissolve/reassemble riding BeamStrike (clone dims mid-flight; bails to clone on any failure; R3F event layer disabled via noopEvents — async connect raced fast unmounts and threw). (3) Cursor parallax (per-wrapper perspective preserved; frozen in camera/beaming; reduced-motion off). Gates: lint clean · 185 tests · build ✓ · desktop/mobile/small walkthroughs 8/8/8 · cross-device E2E (two-page, local transport) XDEV DONE ✓ ×2 · pageerror-free attribution probe. Supabase realtime path feature-complete but UNVERIFIED live from sandbox (*.supabase.co blocked) — verify on the Netlify preview with a real phone.

## Prior round (2026-07-10, round 1, done)
InteractiveShowcase shipped on branch claude/interactive-ar-showcase-pinf3x. New: src/lib/beamGeometry.ts(+10 tests) · src/components/landing/ShowcasePhone.tsx (phone mockup, real booth pipeline, 9:16 letterboxed viewfinder) · src/components/landing/InteractiveShowcase.tsx (state machine idle|camera|beaming|wall, angled WAAPI BeamStrike, LiveWall w/ charged/forward variants, polaroid spring drop w/ alternating tilt, in-scene Capture-again, mobile scroll-follow). Landing.tsx demo section swapped. Per-wrapper CSS perspective (NO shared preserve-3d — depth-sort/Safari hazards), no backdrop-filter on animated 3D elements, legacy gold `.glass` avoided. Gates: lint clean · 173 tests · build ✓ · Playwright fake-camera walkthrough desktop+mobile (2 capture cycles) screenshot-verified. DemoBooth.tsx superseded, unreferenced, NOT deleted (needs user approval).
...
- User (2026-07-10): "Remove the old one for this new effect" — the WAAPI clone image must NOT fly alongside the particles (no doubled photo in flight); "We'll just sprinkle a little bit of our colours as well" — particles are photo-sampled PLUS a sprinkle of SPECTRUM hues.
