...
| ~~P1~~ RESOLVED 2026-07-28 | L3 | `src/pages/Landing.tsx:180` | Premium sells "Priority support" and Deluxe "White-glove setup", but the footer links only Privacy and Terms — no contact route exists anywhere. **Fixed:** a real support desk now exists (`/host/support`, `/admin/support`, a report dialog on every surface) and the footer has a Contact route, so "Priority support" is back on Premium. "White-glove setup" stays OUT — that is a promise about a human doing your setup, and shipping a ticket system does not make it true. |
