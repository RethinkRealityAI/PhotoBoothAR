...
Current state (2026-07-07): PR #9 merged to `main` (de-brand + template onboarding + pricing + guest nav/Challenges/liquid-glass). Active: branch `claude/platform-admin-suite` -> draft PR #10 — platform super-admin suite at `/admin`; Phase 1 done & DEPLOYED, Phases 2-5 pending. Also active: branch `claude/ar-agent-ai-studio-da6d0f` — One-Euro AR tracking smoothing (`src/lib/smoothing.ts`), booth face-tracking hint, AI Event Concierge (chat-driven event creation in `/host/new`, `src/lib/eventDesigner.ts`) rendering A2UI v0.9.1 generative-UI plan cards (`src/lib/a2ui.ts` + `src/components/a2ui/A2uiSurface.tsx`). Live Supabase: migrations `001`-`010` + edge fns `admin-api`, `ai-event-designer` (v3 — GEMINI_API_KEY SET; per-user rate limit via `ai_designer_usage`; client sends the template catalog), `ai-generate-image` (v4 — 3 free frames/event) deployed; `platform_admins` seeded. Branch pushed -> draft PR #13. Stripe keys UNPROVISIONED -> billing returns 503 = the #1 gate before real customers.
