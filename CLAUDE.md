...
Current state (2026-07-07): PR #9 merged. Branch `claude/platform-admin-suite` -> draft PR #10 (admin suite; Phase 1 DEPLOYED, Phases 2-5 pending). Branch `claude/ar-agent-ai-studio-da6d0f` -> draft PR #13 (watched; deploy preview green): One-Euro AR smoothing + booth face hint; AI Event Concierge at `/host/new` (`src/lib/eventDesigner.ts`) rendering A2UI v0.9.1 cards (`src/lib/a2ui.ts` + `src/components/a2ui/A2uiSurface.tsx`; Beamwall widgets TemplatePreview/QrCode/ColorChoice with live accent re-theming); in-flow AI Frame Studio on create-success (`src/pages/host/FrameStudio.tsx` — 9:16 frame -> publish -> booth default); guest `/e/:slug/welcome`; studio Share & Print kit tab; truthful go-live checklist; concierge chat persists (sessionStorage). Live Supabase: migrations `001`-`010`; edge fns `create-event` v5 (platform-admin events -> deluxe = admin god-mode; admin orgs comped 1000cr, all 5 events deluxe), `ai-event-designer` v4 (GEMINI_API_KEY SET; 40/h/user rate limit, admins exempt; client sends template catalog; extraction-first prompt), `ai-generate-image` v4 (3 free frames/event, refund-safe), `admin-api`. Stripe UNPROVISIONED -> billing 503 = #1 gate. Next: human E2E on the PR-13 deploy preview (first live Gemini run), Concierge v3 remainder (draggable frame placement, post-create event-aware chat w/ challenges tools), admin Limits console (suite Phase 4), key rotation+restriction — details docs/STATE.md `## Next` + docs/AGENT-ROADMAP.md.
