...
Current state (2026-07-06): PR #10 merged to `main` (platform super-admin suite Phase 1). Active: branch `claude/beamwall-admin-suite-5muz8t` — the full admin suite (Phases 2-5) is built, deployed, and pending merge/PR review. Live Supabase: migrations `001`-`010` + edge fns `admin-api`/`stripe-webhook` deployed; `platform_admins` seeded. Stripe SANDBOX keys provisioned + validated end-to-end (real test-mode checkout -> webhook signature verify -> `credit_ledger`/`orders` all confirmed) for `credit_pack`; `event_package`/`pro_subscription` still untested. Two real bugs found + fixed this session (see `PROJECT.md` Backlog for the lower-severity items a follow-up security sweep also found, not yet fixed): `fetchMyEvents`/`EventStudio` leaked every customer's events to any signed-in host (now org-scoped + `canEnterStudio` gate) and `Landing`/`Login`/`Signup` clipped content with no scroll (root shell's `h-screen overflow-hidden` vs a nested `min-h-screen` that never bounded height — changed to `h-full`). Next: test `event_package`/`pro_subscription` flows, then swap to LIVE Stripe keys (new webhook endpoint + signing secret) before real customers — the #1 remaining gate.
